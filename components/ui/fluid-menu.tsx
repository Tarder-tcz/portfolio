...
export function MenuItem({ children, onClick, disabled = false, icon, isActive = false, href }: MenuItemProps) {
    const content = (
    ...
    const buttonClasses = `relative block w-full h-16 text-center group
    ...
    if (href) {
        return (
            <Link href={href} className={buttonClasses} role="menuitem" onClick={onClick}>
                {content}
            </Link>
        )
    }
        ...
        <div className="relative w-[64px]" data-expanded={isExpanded}>
            ...
            <div className="relative">
                ...
                {childrenArray.slice(1).map((child, index) => (
                    <div
                        key={index}
                        className="absolute top-0 right-0 w-16 h-16 bg-gray-100 dark:bg-gray-800 will-change-transform rounded-full"
                        style={{
                            transform: `translateY(${isExpanded ? (index + 1) * 80 : 0}px)`,
